...
def fill_mesh(mesh2fill, file: str, opt):
    ...
    if os.path.exists(load_path):
        ...
        # Ensure all arrays have consistent shapes before saving
        mesh_data.gemm_edges = np.array(mesh_data.gemm_edges, dtype=np.int64)
        mesh_data.vs = np.array(mesh_data.vs, dtype=np.float32)
        mesh_data.edges = np.array(mesh_data.edges, dtype=np.int32)
        mesh_data.ve = np.array([np.array(ve, dtype=np.int32) for ve in mesh_data.ve], dtype=object)
        mesh_data.v_mask = np.array(mesh_data.v_mask, dtype=bool)
        mesh_data.edge_lengths = np.array(mesh_data.edge_lengths, dtype=np.float32)
        mesh_data.edge_areas = np.array(mesh_data.edge_areas, dtype=np.float32)
        # Extract features before saving
        try:
            mesh_data.features = extract_features(mesh_data)
        except Exception as e:
            print(f"Error extracting features for {file}: {str(e)}")
            raise
        mesh_data.sides = np.array(mesh_data.sides, dtype=np.int64)
...
def from_scratch(file, opt):
    ...
    mesh_data.vs = None              # Vertex positions (3D coordinates)
    mesh_data.edges = None           # Edge connections between vertices
    mesh_data.gemm_edges = None      # Edge neighborhood information (4 neighbors per edge)
    mesh_data.sides = None           # Side information for edges
    mesh_data.edges_count = None     # Total number of edges
    mesh_data.ve = None             # Vertex-edge connections
    mesh_data.v_mask = None         # Vertex mask (which vertices are active)
    mesh_data.filename = 'unknown'   # Name of the mesh file
    mesh_data.edge_lengths = None    # Length of each edge
    mesh_data.edge_areas = []       # Area associated with each edge
        
    # Fill the vertices and faces information from file
    ...
    # creating neighbourhood ## Explain
